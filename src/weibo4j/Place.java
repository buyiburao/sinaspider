...
public class Place extends Weibo{
	 ...
	 *   注：没有写完
	 */
	public JSONArray category (int pid, int flag) throws WeiboException {
		return client.get(WeiboConfig.getValue("baseURL") + "place/pois/category.json",new PostParameter[] {
	 ...
	 * 注：没写完
	 */
	public JSONObject poisUsersList (String poiid, int page, int count) throws WeiboException {
		return client.get(WeiboConfig.getValue("baseURL") + "place/pois/show.json",new PostParameter[] { 
			new PostParameter("poiid", poiid),
			new PostParameter("page", page),
			new PostParameter("count", count)}).asJSONObject();
	}
